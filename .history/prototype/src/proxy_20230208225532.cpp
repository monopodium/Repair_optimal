...
namespace REPAIR
{
    ...
    bool Proxy::Set(std::string key, std::string value)
    {
        ...
        stripe_this.value_size = value_size_bytes;
        for (int j = 0; j < n; j++)
        {
    ...
    bool Proxy::Get(std::string key, std::string &value)
    {
        ...
        REPAIR::EncodeType encode_type = m_encode_parameter.encodetype;
        REPAIR::PlacementType placement_type = m_encode_parameter.placementtype;
        auto get_from_datanode = [](memcached_st *memcached, const char *key, int key_size, const char *value, int value_size) mutable
        {
            // for(int i = 0;i<value_size;i++){
            //     std::cout<<value[i];
            // }
        ...
        for (int i = 0; i < k; i++)
        {
            int node_id = m_Stripe_info.at(key)[i];
        }
        for (int node_id : m_Stripe_info[key])
        {
        }
    }
