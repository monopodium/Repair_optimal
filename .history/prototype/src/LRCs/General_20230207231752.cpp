...
namespace REPAIR
{
    ...
    std::pair<double, double> Code_Placement::return_DRC_NRC(REPAIR::PlacementType placement_type, int random_seed = 10, bool if_debug = false)
    {
        if (placement_type == REPAIR::Random)
        {
    ...
    {
        for (int i = 0; i < m_raw_stripe.size(); i++)
        {
            ...
            for (auto block : m_block_repair_request[item])
            {
                ...
                repair_set.insert(m_placement_map[block]);
            }
    ...
    void Code_Placement::generate_random_placement(int random_seed = 10)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            std::uniform_real_distribution<int> urd(1, d);
            int blocks_in_cluster = urd(eng);
