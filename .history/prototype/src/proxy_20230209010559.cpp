...
namespace REPAIR
{
    ...
    bool Proxy::Set(std::string key, std::string value)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            if (j < k)
            {
                senders.push_back(std::thread(send_to_datanode, memcached_list[node_id], (const char *)shard_id.data(),
    ...
    bool Proxy::Get(std::string key, std::string &value)
    {
        ...
        REPAIR::EncodeType encode_type = m_encode_parameter.encodetype;
        REPAIR::PlacementType placement_type = m_encode_parameter.placementtype;
        auto get_from_datanode = [](memcached_st *memcached, std::string key, int key_size, char *value) mutable
        {
            // for(int i = 0;i<value_size;i++){
            //     std::cout<<value[i];
            // }
            ...
            char *value_ptr = memcached_get(memcached, (const char*)key, key_size, &value_size, &flag, &error);
            ...
            if (value_ptr == NULL)
            {
                ...
                std::cout << "memcached_get success key:"<<key<< std::endl;
            }
        };
        ...
        for (int i = 0; i < k; i++)
        {
            ...
            receiver.push_back(std::thread(get_from_datanode, memcached_list[node_id], shard_id,
                                           shard_id.size(), data.data()+block_size*i));
        }
        for (int j = 0; j < receiver.size(); j++)
        {
            receiver[j].join();
        }
        std::cout<<"std::cout<<data[i];"<<std::endl;
        ...
        std::cout<<"std::cout<<value<<std::endl;"<<std::endl;
        std::cout<<value_size_bytes<<std::endl;
