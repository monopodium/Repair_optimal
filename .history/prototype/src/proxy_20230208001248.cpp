...
namespace REPAIR
{
    ...
    bool Proxy::Set(std::string key, std::string value)
    {
        ...
        auto encode_and_save = [](memcached_st *memcached, const char *key, int key_size, const char *value, int value_size) mutable
        {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            if (j < k)
            {
                senders.push_back(std::thread(memcached_list[j],(const char *)shard_id.data(),
                shard_id.size(),(const char *)data[j],data[j].size()));
            }
            else
            {
                senders.push_back(std::thread(memcached_list[j], ));
            }
        }
