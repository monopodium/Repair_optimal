...
ARRAY=('10.0.0.51' '10.0.0.52' '10.0.0.53' '10.0.0.54' '10.0.0.55' '10.0.0.56' '10.0.0.58')
...
temp=${ARRAY[$i]}
    ...
    then
        ...
        if [ $1 == 1 ]
        then
            rsync -r ${SRC_PATH} msms@$temp:${DIS_DIR}
        fi
        ssh msms@$temp 'hostname'
        ssh msms@$temp 'cd /home/msms/run_memcached;bash cluster_run_memcached.sh'
...
then
    ...
    if [ $1 == 1 ]
    then
        rsync -r $SRC_PATH msms@$NETWORKCORE_IP:$DIS_DIR
    fi
    ssh msms@$NETWORKCORE_IP 'sudo kill -9 $(pidof memcached)'
    ssh msms@$NETWORKCORE_IP 'cd /home/msms/run_memcached;./memcached/bin/memcached -m 128 -p '$NETWORKCORE_PORT' --max-item-size=5242880 -vv -d'
...
#rsync -r /home/msms/codes/Repair_optimal/prototype/run_memcached msms@192.168.1.121:/home/msms
