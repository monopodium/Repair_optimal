...
namespace REPAIR
{
    ...
    bool Proxy::Get(std::string key, std::string &value)
    {
        ...
        for (int i = 0; i < k; i++)
        {
            ...
            if (i < k)
            {
                receiver.push_back(std::thread(get_from_datanode, memcached_list[j], (const char *)shard_id.data(),
                                              shard_id.size(), (const char *)data[j], block_size));
            }
            else
            {
                senders.push_back(std::thread(get_from_datanode, memcached_list[j], (const char *)shard_id.data(),
                                              shard_id.size(), (const char *)coding[j - k], block_size));
            }
        }
    }
